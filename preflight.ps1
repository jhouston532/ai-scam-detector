...
# One timestamped folder for the whole batch: every site writes into
# <OutDir>\scan-YYYYmmdd-HHmmss\ via the tool's --run-id.
$runId  = "scan-" + (Get-Date -Format "yyyyMMdd-HHmmss")
...
foreach ($url in $urls) {
    ...
    $pyArgs = @($scriptRt, $url, "--outdir", $outRt, "--run-id", $runId)
...
$outShown = $runDir; if (Test-Path -LiteralPath $runDir) { $outShown = (Resolve-Path -LiteralPath $runDir).Path }
Write-Host ("  output  : {0}" -f $outShown)
Write-Host ""
